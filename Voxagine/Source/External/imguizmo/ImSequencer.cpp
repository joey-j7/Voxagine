...
namespace ImSequencer
{
	...
	bool Sequencer(SequenceInterface *sequence, int *currentFrame, bool *expanded, int *selectedEntry, int *firstFrame, int sequenceOptions)
	{
		...
		else
		{
			...
			for (int i = 0; i < sequenceCount; i++)
			{
				uint32_t col = (i & 1) ? 0xFF3A3636 : 0xFF413D3D;
			...
			for (int i = 0; i < sequenceCount; i++)
			{
				...
				uint32_t slotColor = color | 0xFF000000;
				...
				const uint32_t quadColor[] = { 0xFFFFFFFF, 0xFFFFFFFF, slotColor + (selected ? 0 : 0x202020) };
			...
			if (sequenceOptions&SEQUENCER_COPYPASTE)
			{
				...
				uint32_t copyColor = inRectCopy ? 0xFF1080FF : 0xFF000000;
				...
				uint32_t pasteColor = inRectPaste ? 0xFF1080FF : 0xFF000000;
